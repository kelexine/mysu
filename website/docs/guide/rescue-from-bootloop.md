...
In MySU, the following situations may cause boot brick when flashing the boot partition:
...
If you have flashed a module that has been proven to be safe but causes your device to fail to boot, then this situation is easily recoverable in MySU without any worries. MySU has built-in mechanisms to rescue your device, including the following:
...
1. The built-in Safe Mode of some systems: Some systems have a built-in Safe Mode that can be accessed by long-pressing the Volume down button. In other systems (such as MIUI/HyperOS), Safe Mode can be activated from the Recovery. When entering the system's Safe Mode, MySU will also enter this mode and automatically disable the modules.
2. The built-in Safe Mode of MySU: In this case, the method is to **press the Volume down key continuously more than three times** after the first boot screen. Note that it is press-release, press-release, press-release, not hold.

After entering Safe Mode, all modules on the Module page in the MySU manager will be disabled. However, you can still perform the "uninstall" operation to remove any modules that may be causing issues.
...
**Method 1: Use mysud to manage modules via ADB**

If the device can get root shell via ADB, you can use the `mysud` command line directly to disable or uninstall the problematic module:

::: tip
After mounting `metadata` and `data` partitions, you can run `/data/adb/mysud` command under Recovery mode to manage modules.

Since GKI devices share `init`, MySU kernel module will still be loaded under Recovery mode, you should be able to use most features of `mysud` (like setting features) normally.
:::

```
adb shell
su
mysud module list          # List all modules
mysud module disable <id>  # Disable problematic module
mysud module uninstall <id> # Or uninstall directly
reboot
```
...
The module loading of MySU depends on the init.rc injection file on the kernel side and the mysud process in user space. After deleting these files and rebooting, MySU will not load any modules.
...
3. Delete mysud to prevent module loading:
   ```
   rm -f /data/adb/mysud
   ```
4. (Optional) Mount the metadata partition and delete the init.rc injection file generated by the module:
   ```
   mount /metadata
   rm -f /metadata/mysu/modules.rc
   rm -f /metadata/watchdog/mysu/modules.rc
   ```
5. Reboot the device:
   ```
   reboot
...
MySU will skip the loading of all modules after rebooting. After entering the system, you can reopen the MySU manager to deal with the module problems.
