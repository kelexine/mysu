...
Metamodules are a revolutionary feature in MySU that transfers critical module system capabilities from the core to pluggable modules. This architectural shift maintains MySU's stability and security while unleashing greater innovation potential for the module ecosystem.

## What is a Metamodule?

A metamodule is a special type of MySU module that provides core infrastructure functionality for the module system. Unlike regular modules that modify system files, metamodules control *how* regular modules are installed and mounted.

Metamodules are a plugin-based extension mechanism that allows complete customization of MySU's module management infrastructure. By delegating mounting and installation logic to metamodules, MySU avoids being a fragile detection point while enabling diverse implementation strategies.
...
Traditional root solutions bake mounting logic into their core, making them easier to detect and harder to evolve. MySU's metamodule architecture solves these problems through separation of concerns.

**Strategic advantages:**

- **Reduced detection surface**: MySU itself doesn't perform mounts, reducing detection vectors
- **Stability**: Core remains stable while mounting implementations can evolve
- **Innovation**: Community can develop alternative mounting strategies without forking MySU
...
- **Extensibility**: Add features like kernel module support without modifying core MySU
- **Modularity**: Update implementations independently of MySU releases
- **Customization**: Create specialized solutions for specific devices or use cases

::: warning IMPORTANT
Without a metamodule installed, modules will **NOT** be mounted. Fresh MySU installations require installing a metamodule (such as `meta-overlayfs`) for modules to function.
:::
...
You can check which metamodule is currently active in the MySU Manager app's Module page. The active metamodule will be displayed in your module list with its special designation.
...
Only one metamodule can be installed at a time. If you try to install a second metamodule, MySU will prevent the installation to avoid conflicts.
...
If you're developing regular MySU modules, you don't need to worry much about metamodules. Your modules will work as long as users have a compatible metamodule installed (like `meta-overlayfs`).
...
::: tip
If you're familiar with Magisk module development, your modules will work the same way in MySU when metamodule is installed, as it provides Magisk-compatible mounting.
:::

## For Metamodule Developers

Creating a metamodule allows you to customize how MySU handles module installation, mounting, and uninstallation.
...
- All standard MySU environment variables
...
When performing mount operations, you **MUST** set the source/device name to `"MYSU"`. This identifies mounts as belonging to MySU.

**Example (correct):**

```sh
mount -t overlay -o lowerdir=/lower,upperdir=/upper,workdir=/work MYSU /target
```

**For modern mount APIs**, set the source string:

```rust
fsconfig_set_string(fs, "source", "MYSU")?;
```

This is essential for MySU to identify and manage its mounts properly.
:::
...
for module in /data/adb/modules/*; do
    ...
    if [ -d "$module/system" ]; then
        # Mount with source=MYSU (REQUIRED!)
        mount -o bind,dev=MYSU "$module/system" /system
    fi
done
```
...
- **Variables**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `MYSU`, `MYSU_VER`, `MYSU_VER_CODE`, `MYSU_UAPI_VER`, `MYSU_RUNTIME_MODE`, `MYSU_LATE_LOAD`, `BOOTMODE`, etc.
- **Functions**:
...
When a metamodule is installed, MySU creates a symlink:
...
fsconfig_set_string(fs, "source", "MYSU")?;  // REQUIRED!
```

This sets `dev=MYSU` for all overlay mounts, enabling proper identification.
...
1. **Test installation** on a clean MySU setup
...
- [Difference with Magisk](difference-with-magisk.md) - Comparing MySU and Magisk
- [How to Build](how-to-build.md) - Building MySU from source
