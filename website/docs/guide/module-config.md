...
MySU provides a built-in configuration system that allows modules to store persistent or temporary key-value settings. Configurations are stored in a binary format at `/data/adb/mysu/module_configs/<module_id>/` with the following characteristics:
...
All module scripts (`post-fs-data.sh`, `service.sh`, `boot-completed.sh`, etc.) run with the `MYSU_MODULE` environment variable set to the module ID. You can use the `mysud module config` commands to manage your module's configuration:

```bash
# Get a configuration value
value=$(mysud module config get my_setting)

# Set a persistent configuration value
mysud module config set my_setting "some value"

# Set a temporary configuration value (cleared on reboot)
mysud module config set --temp runtime_state "active"

# Set value from stdin (useful for multiline or complex data)
mysud module config set my_key <<EOF
multiline
text value
EOF

# Or pipe from command
echo "value" | mysud module config set my_key

# Explicit stdin flag
cat file.json | mysud module config set json_data --stdin

# List all configuration entries (merged persist + temp)
mysud module config list

# Delete a configuration entry
mysud module config delete my_setting

# Delete a temporary configuration entry
mysud module config delete --temp runtime_state

# Clear all persistent configurations
mysud module config clear

# Clear all temporary configurations
mysud module config clear --temp
```
...
- Configurations are stored in a binary format with magic number `0x4d53554d` ("MSUM") and version validation
...
- Use the `mysud module config list` command to debug configuration issues
:::
...
mysud module config set override.description "Custom description shown in the manager"
```
...
Modules can declare which MySU features they manage using the `manage.<feature>` configuration pattern. The supported features correspond to MySU's internal `FeatureId` enum:
...
mysud module config delete manage.su_compat
```
...
- MySU manager to detect which modules manage which MySU features
