...
pub fn init() -> Result<()> {
    ...
    if mysuinit::has_mysu() {
        ...
        if let Err(e) = load_module_from_path("/mysu.ko") {
            log::error!("Cannot load mysu.ko: {:?}", e);
        }
    }
...
fn load_module_from_path(path: &str) -> Result<()> {
    ...
    let params = std::fs::read("/mysu_config").unwrap_or_default();
    let params = unsafe { CString::from_vec_unchecked(params) };
    log::info!("load mysu with params {params:?}");
    mysuinit::load_module(&buffer, &params)
}
