...
fn has_mysu_v2() -> bool {
    use syscalls::{Sysno, syscall};
    const MYSU_INSTALL_MAGIC1: u32 = 0xDEADBEEF;
    const MYSU_INSTALL_MAGIC2: u32 = 0xCAFEBABE;
    const MYSU_IOCTL_GET_INFO: u32 = 0x80104b02; // _IOR('K', 2, struct mysu_get_info_cmd)
    const MYSU_IOCTL_GET_INFO_LEGACY: u32 = 0x80004b02; // _IOC(_IOC_READ, 'K', 2, 0)
    ...
    let version = if fd >= 0 {
        ...
        let version = unsafe {
            let ret = syscall!(Sysno::ioctl, fd, MYSU_IOCTL_GET_INFO, &mut cmd as *mut _);
...
pub fn has_mysu() -> bool {
    has_mysu_v2() || has_mysu_legacy()
}
