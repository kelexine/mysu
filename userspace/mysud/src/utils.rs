...
use crate::defs::MYSU_TEMP_BACKUP_DIR_NAME;
use crate::{assets, boot_patch, defs, module, mysucalls, restorecon};
...
pub fn is_safe_mode() -> bool {
    ...
    let safemode = mysucalls::check_kernel_safemode();
...
fn link_mysud_to_bin() -> Result<()> {
    let mysu_bin = PathBuf::from(defs::DAEMON_PATH);
    let mysu_bin_link = PathBuf::from(defs::DAEMON_LINK_PATH);
    if mysu_bin.exists() && !mysu_bin_link.exists() {
        std::os::unix::fs::symlink(&mysu_bin, &mysu_bin_link)?;
    }
    Ok(())
}
...
pub fn install(libadbroot: Option<PathBuf>, data_path: Option<PathBuf>) -> Result<()> {
    ...
    std::fs::copy(
        ...
        // So that if someone execute /data/adb/mysud install, mysud won't be removed unexpectedly
        "/proc/self/exe",
        defs::DAEMON_PATH,
    )?;
    restorecon::lsetfilecon(defs::DAEMON_PATH, restorecon::MYSU_CON)?;
    ...
    if let Some(data_path) = data_path {
        let backup_path = data_path.join(MYSU_TEMP_BACKUP_DIR_NAME);
        if backup_path.is_dir() {
            for ent in backup_path.read_dir()? {
                let ent = ent?;
                if ent.file_type().is_ok_and(|v| v.is_file()) {
                    let name = ent.file_name().to_string_lossy().to_string();
                    let target = format!("{}{name}", defs::MYSU_BACKUP_DIR);
                    if name.starts_with(defs::MYSU_BACKUP_FILE_PREFIX)
...
pub fn uninstall(package_name: &str) -> Result<()> {
    ...
    println!("- Uninstall MySU manager..");
    Command::new("pm")
...
pub fn detach_process_group(use_init_pgrp: bool) {
    if use_init_pgrp {
        if let Err(e) = mysucalls::set_init_pgrp() {
