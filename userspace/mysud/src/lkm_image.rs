...
const CAPSULE_MAGIC: &[u8; 8] = b"MSULKM1\0";
...
pub struct BootPatchV2Args {
    ...
    /// Exact mysu.ko; auto-select the embedded KMI build when omitted
    #[arg(short, long)]
...
impl<'a> BootstrapObject<'a> {
    fn parse(data: &'a [u8]) -> Result<Self> {
        ...
        let text_index = find_unique_section(".text.mysu_bootstrap")?;
        let rodata_index = find_unique_section(".rodata.mysu_bootstrap")?;
    ...
    fn link(
        ...
        Ok(LinkedBootstrap {
            entry_address: self.named_symbol_value("mysu_bootstrap", code_address, definitions)?,
...
fn inject_image(original_image: &[u8], module: &[u8]) -> Result<(Vec<u8>, ImageInjectionReport)> {
    ...
    definitions.insert(
        "mysu_ext_memblock_reserve",
        sites.memblock_reserve_call.target,
    );
    definitions.insert("mysu_ext_memstart_addr", required.memstart_addr.address);
    definitions.insert("mysu_ext_kimage_voffset", required.kimage_voffset.address);
    definitions.insert("mysu_ext_async_synchronize_full", sites.async_call.target);
    definitions.insert("mysu_ext_vmalloc", required.vmalloc.address);
    definitions.insert("mysu_ext_memcpy", required.memcpy.address);
    definitions.insert("mysu_ext_load_module", required.load_module.address);
    definitions.insert("mysu_ext_kstrdup", required.kstrdup.address);
    definitions.insert("mysu_ext_strndup_user", sites.strndup_call.target);
    definitions.insert("mysu_image_base", image_base);
    definitions.insert("mysu_capsule_magic", u64::from_le_bytes(*CAPSULE_MAGIC));
    definitions.insert("mysu_capsule_version", u64::from(CAPSULE_VERSION));
    definitions.insert("mysu_capsule_header_size", CAPSULE_HEADER_SIZE as u64);
    definitions.insert("mysu_capsule_image_offset", capsule.file_offset as u64);
    definitions.insert("mysu_capsule_size", capsule.data.len() as u64);
    definitions.insert(
        "mysu_module_capsule_offset",
        capsule.module_offset.saturating_sub(capsule.file_offset) as u64,
    );
    definitions.insert(
        "mysu_fixup_capsule_offset",
        capsule.fixup_offset.saturating_sub(capsule.file_offset) as u64,
    );
    definitions.insert("mysu_reserve_extension", reserve_extension as u64);
    definitions.insert("mysu_page_offset", sites.page_offset);
    definitions.insert("mysu_module_size", module.len() as u64);
    definitions.insert("mysu_fixup_count", fixups.len() as u64);
    definitions.insert("mysu_load_info_size", gki_abi.load_info_storage_size);
    definitions.insert("mysu_load_info_hdr_offset", gki_abi.load_info_hdr_offset);
    definitions.insert("mysu_load_info_len_offset", gki_abi.load_info_len_offset);
    definitions.insert("mysu_gfp_kernel", gki_abi.gfp_kernel);
...
pub fn patch_boot(args: &BootPatchV2Args) -> Result<()> {
    ...
    let module = if let Some(module) = &args.module {
        ...
        assets::get_asset_data(&name)
            .with_context(|| format!("no embedded MySU module for KMI {kmi}: {name}"))?
            .into_owned()
    };
...
mod tests {
    ...
    fn bootstrap_test_definitions<'a>(
        ...
        for (index, symbol) in object
            ...
            .enumerate()
        {
            let value = if symbol.name.starts_with("mysu_ext_") || symbol.name == "mysu_image_base"
            {
    ...
    fn embedded_module_uses_release_asset_layout() {
        ...
        #[cfg(target_os = "android")]
        assert_eq!(name, "android12-5.10_mysu.ko");
        #[cfg(not(target_os = "android"))]
        assert_eq!(name, "aarch64/android12-5.10_mysu.ko");
