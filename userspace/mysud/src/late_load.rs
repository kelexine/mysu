...
pub fn run(package_name: &String, kmi: Option<String>, allow_shell: bool) -> Result<()> {
    ...
    if mysuinit::has_kernelsu() {
        ...
        // 4. Load mysu.ko from memory with manual relocation
        info!("Loading mysu.ko for KMI {kmi}...");
        let params = if allow_shell {
            cstr!("allow_shell=1")
        } else {
            cstr!("")
        };
        mysuinit::load_module(&ko_data, params).context("Failed to load mysu.ko")?;
        info!("mysu.ko loaded successfully!");
    ...
    // will be blocked by SELinux because its fsec->sid is still u:r:su:s0 instead of u:r:mysu:s0.
    utils::reset_std()?;
    ...
    utils::install(None, None).context("Failed to install mysud")?;
    ...
    // 14. Restart Manager so it gets a fresh mysu fd from the newly loaded kernel module
    info!("Restarting MySU Manager {package_name}...");
    ...
    let _ = Command::new("am")
        .args([
            "start",
            "-n",
            &format!("{package_name}/dev.kelexine.mysu.ui.MainActivity"),
        ])
        .status();
