...
/// Find PIDs of processes running in the MySU su domain (u:r:mysu:s0).
...
/// Find PIDs of processes holding mysu_driver or mysu_fdwrapper file descriptors.
/// Returns a list of PIDs excluding our own.
fn find_mysu_fd_holders() -> Vec<i32> {
    let my_pid = std::process::id() as i32;
    let mut pids = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        for fd_entry in fds.flatten() {
            ...
            if let Ok(target) = fs::read_link(&link_path) {
                ...
                if target_str.contains("[mysu_driver]") || target_str.contains("[mysu_fdwrapper]") {
                    pids.push(pid);
                    break;
                }
            }
...
/// Close all mysu_driver and mysu_fdwrapper fds held by the current process.
fn close_mysu_fds() {
...
pub fn unload() -> Result<()> {
    info!("unload: starting MySU unload sequence");
    ...
    // 2. Kill all su domain processes and processes holding mysu fds (except ourselves)
    ...
    info!("unload: killing processes holding mysu fds...");
    let fd_pids = find_mysu_fd_holders();
    if !fd_pids.is_empty() {
        info!(
            "unload: found {} processes holding mysu fds, sending SIGKILL",
            fd_pids.len()
        );
        kill_pids(&fd_pids, libc::SIGKILL);
    }

    // 3. Close all our own mysu_driver and mysu_fdwrapper fds
    info!("unload: closing all mysu fds...");
    close_mysu_fds();

    // 4. delete_module("mysu")
    info!("unload: removing mysu module...");
    if let Err(e) = rustix::system::delete_module(c"mysu", 0) {
        warn!("unload: delete_module mysu failed: {e}");
    }
    ...
    // 6. Exit
    info!("unload: done, exiting mysud");
    std::process::exit(0);
}
