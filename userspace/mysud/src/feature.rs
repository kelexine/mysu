...
impl FeatureId {
    ...
    pub const fn description(self) -> &'static str {
        match self {
            ...
            Self::VfsHide => "VFS Hide - cloaks /data/adb and module paths from denied UIDs",
        }
    }
}
...
fn parse_feature_id(name: &str) -> Result<FeatureId> {
    match name {
        ...
        "vfs_hide" | "5" => Ok(FeatureId::VfsHide),
        _ => bail!("Unknown feature: {name}"),
    }
}

fn set_kernel_feature(feature_id: FeatureId, value: u64) -> Result<()> {
    crate::mysucalls::set_feature(feature_id as u32, value)
...
pub fn apply_config(features: &HashMap<u32, u64>) {
    ...
    for (&id, &value) in features {
        match FeatureId::from_u32(id) {
            ...
            None => match crate::mysucalls::set_feature(id, value) {
                Ok(()) => {
...
pub fn get_feature(id: &str) -> Result<()> {
    ...
    let (value, supported) = crate::mysucalls::get_feature(feature_id as u32)
...
pub fn set_feature(id: &str, value: u64) -> Result<()> {
    ...
    if let Ok(managed_features_map) = crate::module::get_managed_features() {
        ...
        if !managing_modules.is_empty() {
            ...
            let caller_module = std::env::var("MYSU_MODULE").unwrap_or_default();
...
pub fn list_features() {
    ...
    for feature_id in &all_features {
        ...
        let (value, supported) = crate::mysucalls::get_feature(id).unwrap_or((0, false));
...
pub fn save_config() -> Result<()> {
    ...
    for feature_id in &all_features {
        let id = *feature_id as u32;
        if let Ok((value, supported)) = crate::mysucalls::get_feature(id)
            && supported
        {
            features.insert(id, value);
...
pub fn check_feature(id: &str) -> Result<()> {
    ...
    let (_value, supported) = crate::mysucalls::get_feature(feature_id as u32)
        .with_context(|| format!("Failed to get feature {id}"))?;
