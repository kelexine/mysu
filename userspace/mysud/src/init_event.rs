...
use crate::{
    assets, defs, metamodule, mysucalls, restorecon,
    utils::{self},
};
...
pub fn on_post_data_fs() -> Result<()> {
    if let Err(e) = mysucalls::ensure_uapi_version_matched() {
        error!("{e:#}, skip on_post_fs_data");
        return Ok(());
    }

    mysucalls::report_post_fs_data();
    ...
    // Refresh /metadata/watchdog/mysu/modules.rc so the next boot's kernel hook sees the
    // current module set. Acts as a safety net when state was changed outside
    // of mysud's normal mutation commands.
...
pub fn on_boot_completed() {
    ...
    mysucalls::report_boot_complete();
