...
use crate::{
    apk_sign, assets, debug, defs, init_event, module, module_config, mysu_uapi, mysucalls, sulog,
    utils,
};
...
enum Commands {
    ...
    /// Run sulog reader daemon. Not for user. Use `mysud debug sulogd` to launch daemon.
    #[command(hide = true)]
    Sulogd,
    ...
    /// Install MySU userspace component to system
    Install {
    ...
    /// Unload MySU kernel module (LKM Only)
    Unload,

    /// Uninstall MySU modules and itself(LKM Only)
    Uninstall {
        #[arg(long, default_value_t = String::from(defs::DEFAULT_PACKAGE_NAME))]
        package_name: String,
    },
    ...
    /// Restore boot or init_boot images patched by MySU
    BootRestore(BootRestoreArgs),

    /// Patch MySU into a boot image
    ///
...
#[derive(clap::Subcommand, Debug)]
enum Debug {
    /// Set the manager app, kernel CONFIG_MYSU_DEBUG should be enabled.
    SetManager {
        /// manager package name
...
enum Feature {
    ...
    Get {
        /// Feature ID or name (su_compat, kernel_umount, sulog, adb_root, selinux_hide, vfs_hide)
        id: String,
...
pub fn run() -> Result<()> {
    ...
    let result = match cli.command {
            ...
            match command {
                ...
                Module::Config { internal, command } => {
                    let module_id = match internal {
                        ...
                        None => std::env::var("MYSU_MODULE").map_err(|_| {
                            anyhow::anyhow!(
        ...
        Commands::Services => {
            if mysucalls::get_version() <= 0 {
                info!("MySU not available, exiting services");
                std::process::exit(0);
            }
        ...
        Commands::Debug { command } => match command {
            ...
            Debug::Version => {
                println!("Kernel Version: {}", mysucalls::get_version());
                Ok(())
            }
            ...
            Debug::Info => {
                let info = mysucalls::get_info();
                println!("version: {}", info.version);
                println!("flags: 0x{:x}", info.flags);
                println!("uapi_version: {}", info.uapi_version);
                println!("features: 0x{:x}", info.features);
                println!("lkm: {}", mysucalls::is_lkm());
                println!("late_load: {}", mysucalls::is_late_load());
                println!("runtime_mode: {}", mysucalls::runtime_mode());
                println!(
                    "pr_build: {}",
                    (info.flags & mysu_uapi::MYSU_GET_INFO_FLAG_PR_BUILD) != 0
                );
                Ok(())
            }
            Debug::Package => {
        ...
        Commands::Kernel { command } => match command {
            Kernel::NukeExt4Sysfs { mnt } => mysucalls::nuke_ext4_sysfs(&mnt),
            Kernel::Umount { command } => match command {
                UmountOp::Add { mnt, flags } => mysucalls::umount_list_add(&mnt, flags),
                UmountOp::Del { mnt } => mysucalls::umount_list_del(&mnt),
                UmountOp::Wipe => mysucalls::umount_list_wipe().map_err(Into::into),
            },
            Kernel::NotifyModuleMounted => {
                mysucalls::report_module_mounted();
                Ok(())
            }
        },
