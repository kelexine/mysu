...
use crate::utils::*;
use crate::{
    assets, defs, metamodule, mysucalls,
...
pub fn get_common_script_envs(module_id: Option<&str>) -> Vec<(&'static str, String)> {
    let mut envs = vec![
        ...
        ("MYSU", "true".to_string()),
        ("MYSU_KERNEL_VER_CODE", mysucalls::get_version().to_string()),
        ("MYSU_VER_CODE", defs::VERSION_CODE.to_string()),
        ("MYSU_VER", defs::VERSION_NAME.to_string()),
        ("MYSU_UAPI_VER", mysucalls::uapi_version().to_string()),
        ("MYSU_RUNTIME_MODE", mysucalls::runtime_mode().to_string()),
        (
            "PATH",
            format!(
                "{}:{}",
    ...
    if let Some(id) = module_id {
        if validate_module_id(id).is_ok() {
            envs.push(("MYSU_MODULE", id.to_string()));
        } else {
            error!("Invalid module_id provided: {id}");
        }
    }

    if mysucalls::is_late_load() {
        envs.push(("MYSU_LATE_LOAD", "1".to_string()));
    }
...
pub fn exec_script<T: AsRef<Path>>(path: T, wait: bool) -> Result<()> {
    ...
    if is_module_script && module_id.is_none() {
        debug!(
            "Failed to extract module_id from script path '{}'. Script will run without MYSU_MODULE environment variable.",
            path.as_ref().display()
        );
    }
...
fn preinit_mysu_dir() -> &'static str {
...
pub fn regenerate_preinit_rc() -> Result<()> {
    let preinit_str = preinit_mysu_dir();
...
pub fn install_module(zip: &str) -> Result<()> {
    mysucalls::ensure_uapi_version_matched()?;
