...
mod android {
    ...
    pub fn ensure_binaries(ignore_if_exist: bool) -> anyhow::Result<()> {
        for file in Asset::iter() {
            if file == "mysuinit" || file.ends_with(".ko") {
                // don't extract mysuinit and kernel modules
                continue;
            }
            let asset =
        ...
        // Create resetprop -> mysud symlink (resetprop is now built into mysud)
        let resetprop_link = RESETPROP_PATH;
        let _ = std::fs::remove_file(resetprop_link);
        std::os::unix::fs::symlink("/data/adb/mysud", resetprop_link)?;
