...
mod android {
    use super::Result;
    pub(super) use crate::defs::{BACKUP_FILENAME, MYSU_BACKUP_DIR, MYSU_BACKUP_FILE_PREFIX};
    use crate::defs::{DEFAULT_PACKAGE_NAME, MYSU_TEMP_BACKUP_DIR_NAME};
    use android_bootimg::cpio::{Cpio, CpioEntry};
    ...
    fn find_backup_location(sha1: &String) -> Result<(File, String)> {
        let filename = format!("{MYSU_BACKUP_FILE_PREFIX}{sha1}");
        let target = format!("{MYSU_BACKUP_DIR}{filename}");
        ...
        let backup_dir =
            format!("/data/user_de/{user_id}/{DEFAULT_PACKAGE_NAME}/{MYSU_TEMP_BACKUP_DIR_NAME}");
        ...
        bail!("Both /data/adb/mysu and {backup_dir} are not accessible!")
    }
    ...
    pub(super) fn clean_backup(sha1: &str) -> Result<()> {
        ...
        let backup_name = format!("{MYSU_BACKUP_FILE_PREFIX}{sha1}");
        let dir = std::fs::read_dir(MYSU_BACKUP_DIR)?;
        for entry in dir.flatten() {
            ...
            if let Some(name) = path.file_name() {
                ...
                if name != backup_name
                    && name.starts_with(MYSU_BACKUP_FILE_PREFIX)
...
pub struct BootPatchArgs {
    ...
    /// Do not (re-)install mysu, only modify configs (allow_shell, etc.)
    #[arg(long, default_value = "false")]
    no_install: bool,
...
pub fn patch(args: BootPatchArgs) -> Result<()> {
    let inner = move || {
        ...
        let mysu_init: Box<dyn AsRef<[u8]>> = if no_install {
            Box::new(Vec::<u8>::new())
            ...
            #[cfg(not(target_os = "android"))]
            {
                assets::get_asset(&format!("{arch}/mysuinit")).context("Failed to load mysuinit")?
            }
            #[cfg(target_os = "android")]
            {
                assets::get_asset("mysuinit").context("Failed to load mysuinit")?
            }
        };
        ...
        let mut mysu_config: Vec<String> = cpio
            .entry_by_name("mysu_config")
        ...
        let mut apply_config = |name: &str, value: &str, add: bool| {
            let has_value = mysu_config.iter().any(|v| v == value);

            if add {
                println!("- Adding {name} config");
                if !has_value {
                    mysu_config.push(value.to_owned());
                }
            } else if has_value {
                println!("- Removing {name} config");
                mysu_config.retain(|v| v != value);
            }
        };
        ...
        if mysu_config.is_empty() {
            cpio.rm("mysu_config", false);
        } else {
            let data = mysu_config.join(" ").into_bytes();
            cpio.add("mysu_config", CpioEntry::regular(0o644, Box::new(data)))?;
        }
...
pub fn restore(args: BootRestoreArgs) -> Result<()> {
    ...
    ensure!(cpio.exists("mysu.ko"), "boot image is not patched by MySU");
    ...
    if let Some(backup_file) = cpio.entry_by_name(BACKUP_FILENAME) {
        ...
        let sha = sha.trim();
        let backup_path =
            PathBuf::from(MYSU_BACKUP_DIR).join(format!("{MYSU_BACKUP_FILE_PREFIX}{sha}"));
    ...
    let new_boot_bytes: Vec<u8> = {
        #[cfg(target_os = "android")]
        {
            if let Some(stock_path) = stock_boot {
                ...
                rebuild_without_mysu(&boot_image, &mut cpio, vendor_ramdisk_idx)?
            }
        }
        #[cfg(not(target_os = "android"))]
        {
...
fn rebuild_without_mysu(
    boot_image: &BootImage<'_>,
    cpio: &mut Cpio,
    vendor_ramdisk_idx: Option<usize>,
) -> Result<Vec<u8>> {
    println!("- Removing MySU from boot image");
    cpio.rm("mysu.ko", false);
