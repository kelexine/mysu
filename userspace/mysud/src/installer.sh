...
check_sepolicy() {
    /data/adb/mysud sepolicy check "$1"
    return $?
}
...
check_managed_features() {
  ...
  echo "$MANAGED_FEATURES" | tr ',' '\n' | while read -r feature; do
    ...
    # Check feature status using mysud
    local status=$(/data/adb/mysud feature check "$feature" 2>/dev/null)
...
install_module() {
  ...
  # Programmatic module adaptation for MySU (No symlinks needed)
  adapt_script() {
    [ -f "$1" ] || return 0
    sed -i 's|/data/adb/ksu/|/data/adb/mysu/|g' "$1" 2>/dev/null || true
    sed -i 's|/data/adb/ksu\b|/data/adb/mysu|g' "$1" 2>/dev/null || true
    sed -i 's|/data/adb/modules/|/data/adb/mysu/modules/|g' "$1" 2>/dev/null || true
    sed -i 's|/data/adb/modules\b|/data/adb/mysu/modules|g' "$1" 2>/dev/null || true
    sed -i 's|/data/adb/ksud|/data/adb/mysud|g' "$1" 2>/dev/null || true
    sed -i 's|\bksud\b|mysud|g' "$1" 2>/dev/null || true
  }
  adapt_script "$MODPATH/post-fs-data.sh"
  adapt_script "$MODPATH/service.sh"
  adapt_script "$MODPATH/action.sh"
  adapt_script "$MODPATH/boot-completed.sh"
  adapt_script "$MODPATH/uninstall.sh"
