...
use crate::mysucalls::get_wrapped_fd;
use rustix::{
    process::getuid,
...
pub fn root_shell() -> Result<()> {
    ...
    opts.optflag("W", "no-wrapper", "don't use mysu fd wrapper");
    opts.optflag(
        "",
        "mysu-no-new-privs",
        "Prevent this process (and its children) from privilege re-escalation via MySU",
    );
    ...
    if matches.opt_present("v") {
        println!("{}:MySU", defs::VERSION_NAME);
        return Ok(());
    }
    ...
    let mysu_no_new_privs = matches.opt_present("mysu-no-new-privs");
    ...
    // when MYSURC_PATH exists and ENV is not set, set ENV to MYSURC_PATH
    if PathBuf::from(defs::MYSURC_PATH).exists() && env::var("ENV").is_err() {
        command.env("ENV", defs::MYSURC_PATH);
    }

    if mysu_no_new_privs {
        mysucalls::set_mysu_no_new_privs().context("set MYSU_NO_NEW_PRIVS")?;
    }
