...
fn assemble_bootstrap() {
    ...
    println!("cargo:rerun-if-env-changed=MYSU_LKM_BOOTSTRAP_OBJECT");
    println!("cargo:rerun-if-env-changed=MYSU_LKM_BOOTSTRAP_CC");
    ...
    if let Some(prebuilt) = env::var_os("MYSU_LKM_BOOTSTRAP_OBJECT") {
        let prebuilt = PathBuf::from(prebuilt);
        copy_bootstrap_object(&prebuilt, &output).unwrap_or_else(|error| {
            panic!(
                "cannot use MYSU_LKM_BOOTSTRAP_OBJECT {}: {error}",
                prebuilt.display()
            )
        });
        return;
    ...
    if let Some(compiler) = env::var_os("MYSU_LKM_BOOTSTRAP_CC") {
    ...
    match run_assembler(&llvm_mc, &llvm_arguments) {
        ...
        Err(error) => {
            errors.push(error);
            panic!(
                "cannot assemble the AArch64 LKM bootstrap; install an AArch64 GNU compiler, clang, or llvm-mc, or set MYSU_LKM_BOOTSTRAP_OBJECT:\n{}",
                errors.join("\n")
            );
        }
    }
...
fn main() {
    ...
    if env::var("MYSU_PACKAGE_NAME").is_err() {
        println!("cargo:rustc-env=MYSU_PACKAGE_NAME=dev.kelexine.mysu");
    }
