...
val defaultManagerPackageName = if (isPrBuild) "dev.kelexine.mysu.pr" else "dev.kelexine.mysu"
val defaultManagerName = if (isPrBuild) "MySU PR" else "MySU"
val managerPackageName = project.findProperty("MYSU_PACKAGE_NAME")?.toString() ?: defaultManagerPackageName
val managerName = project.findProperty("MYSU_NAME")?.toString() ?: defaultManagerName

for (prop in listOf("KEYSTORE_FILE", "KEYSTORE_PASSWORD", "KEY_ALIAS", "KEY_PASSWORD")) {
    if (!project.hasProperty(prop)) {
        val envVal = System.getenv(prop) ?: System.getenv("ORG_GRADLE_PROJECT_$prop") ?: System.getenv("MYSU_$prop")
        if (envVal != null) {
            project.extensions.extraProperties.set(prop, envVal)
        }
    }
}
