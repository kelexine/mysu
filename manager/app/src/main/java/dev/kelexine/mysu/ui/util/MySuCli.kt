...
import dev.kelexine.mysu.core.tasks.BootKernelVersion
import dev.kelexine.mysu.core.tasks.ExtractImage
import dev.kelexine.mysu.core.tasks.ProbeResult
import dev.kelexine.mysu.core.utils.DataSourceChannel
...
private fun getMySuDaemonPath(): String {
    return mysuApp.applicationInfo.nativeLibraryDir + File.separator + "libmysud.so"
}
...
fun getRootShell(globalMnt: Boolean = false): Shell {
    return if (globalMnt) MySuCli.GLOBAL_MNT_SHELL else {
        MySuCli.SHELL
    }
}
...
fun createRootShell(globalMnt: Boolean = false): Shell {
    ...
    return try {
        if (globalMnt) {
            builder.build(getMySuDaemonPath(), "debug", "su", "-g")
        } else {
            builder.build(getMySuDaemonPath(), "debug", "su")
        }
    } catch (e: Throwable) {
        Log.w(TAG, "mysu failed: ", e)
        try {
            if (globalMnt) {
                builder.build("su", "-mm")
            } else {
...
fun execMySud(args: String, newShell: Boolean = false, globalMnt: Boolean = false): Boolean {
    return if (newShell) {
        withNewRootShell(globalMnt = globalMnt) {
            ShellUtils.fastCmdResult(this, "${getMySuDaemonPath()} $args")
        }
    } else {
        ShellUtils.fastCmdResult(getRootShell(globalMnt), "${getMySuDaemonPath()} $args")
    }
}
...
suspend fun getFeatureStatus(feature: String): String = withContext(Dispatchers.IO) {
    ...
    val out = shell.newJob()
        .add("${getMySuDaemonPath()} feature check $feature").to(ArrayList<String>(), null).exec().out
...
suspend fun getFeaturePersistValue(feature: String): Long? = withContext(Dispatchers.IO) {
    val shell = getRootShell()
    val out = shell.newJob()
        .add("${getMySuDaemonPath()} feature get --config $feature").to(ArrayList<String>(), null).exec().out
...
fun install() {
    ...
    val libadbroot = File(mysuApp.applicationInfo.nativeLibraryDir, "libadbroot.so").absolutePath
    val result = execMySud("install --libadbroot $libadbroot --data-path ${mysuApp.applicationInfo.deviceProtectedDataDir}", true)
...
fun listModules(): String {
    ...
    val out = shell.newJob()
        .add("${getMySuDaemonPath()} module list").to(ArrayList(), null).exec().out
...
fun flashModule(
    ...
    val resolver = mysuApp.contentResolver
    with(resolver.openInputStream(uri)) {
        val file = File(mysuApp.cacheDir, "module.zip")
        file.outputStream().use { output ->
            this?.copyTo(output)
        }
        val cmd = "module install ${file.absolutePath}"
        val result = flashWithIO("${getMySuDaemonPath()} $cmd", onStdout, onStderr)
        Log.i("MySU", "install module $uri result: $result")
...
fun runModuleAction(
    ...
    val result = withNewRootShell(true) {
        newJob().add("${getMySuDaemonPath()} module action $moduleId")
            .to(stdoutCallback, stderrCallback).exec()
    }

    Log.i("MySU", "Module runAction result: $result")
...
fun restoreBoot(
    ...
    val result = flashWithIO("${getMySuDaemonPath()} boot-restore -f", onStdout, onStderr)
    return FlashResult(result)
}
...
fun uninstallPermanently(
    ...
    val result = flashWithIO("${getMySuDaemonPath()} uninstall --package-name ${BuildConfig.APPLICATION_ID}", onStdout, onStderr)
    return FlashResult(result)
}
...
private fun writeLkmFile(lkm: LkmSelection): File? {
    ...
    val file = File(mysuApp.cacheDir, "mysu-tmp-lkm.ko")
    mysuApp.contentResolver.openInputStream(lkm.uri)?.use { input ->
...
fun installBoot(
    ...
    val bootFile = bootUri?.let { uri ->
        with(resolver.openInputStream(uri)) {
            val bootFile = File(mysuApp.cacheDir, "boot.img")
    ...
    val result = flashWithIO("${getMySuDaemonPath()} $cmd", onStdout, onStderr)
    Log.i("MySU", "install boot result: ${result.isSuccess}")
    ...
    if (showReboot) { // because we decide do not update mysud when startActivity
        install() // install mysud here
    }
...
fun downloadBoot(
    ...
    val bootFile = File(mysuApp.cacheDir, "download-boot.img")
    ...
    // is unrelated to this device, so mysud must not use the local kernel.
    ...
    var cmd = "${getMySuDaemonPath()} boot-patch -b ${bootFile.absolutePath}"
    cmd += bootPatchFlags(allowShell, enableAdb, forceBackup)
    ...
    // mysud defaults to cwd, which is read-only in the su session; use Downloads.
    val downloadsDir =
        Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
    cmd += " -o $downloadsDir"
...
fun reboot(reason: String = "") {
    if (reason == "soft_reboot") {
        execMySud("soft-reboot", true, true)
        return
    }
    val shell = getRootShell()
    if (reason == "recovery") {
...
suspend fun getCurrentKmi(): String = withContext(Dispatchers.IO) {
    ...
    ShellUtils.fastCmd(shell, "${getMySuDaemonPath()} $cmd")
}

suspend fun getSupportedKmis(): List<String> = withContext(Dispatchers.IO) {
    val shell = getRootShell()
    val cmd = "boot-info supported-kmis"
    val out = shell.newJob().add("${getMySuDaemonPath()} $cmd").to(ArrayList(), null).exec().out
...
suspend fun isAbDevice(): Boolean = withContext(Dispatchers.IO) {
    ...
    ShellUtils.fastCmd(shell, "${getMySuDaemonPath()} $cmd").trim().toBoolean()
}
...
suspend fun getDefaultPartition(): String = withContext(Dispatchers.IO) {
    ...
    if (shell.isRoot) {
        ...
        ShellUtils.fastCmd(shell, "${getMySuDaemonPath()} $cmd").trim()
    } else {
...
fun isSepolicyValid(rules: String?): Boolean {
    ...
    val shell = getRootShell()
    val result =
        shell.newJob().add("${getMySuDaemonPath()} sepolicy check '$rules'").to(ArrayList(), null)
            .exec()
    return result.isSuccess
}
...
fun getSepolicy(pkg: String): String {
    val shell = getRootShell()
    val result =
        shell.newJob().add("${getMySuDaemonPath()} profile get-sepolicy $pkg").to(ArrayList(), null)
            .exec()
...
fun setSepolicy(pkg: String, rules: String): Boolean {
    val shell = getRootShell()
    val result = shell.newJob().add("${getMySuDaemonPath()} profile set-sepolicy $pkg '$rules'")
...
fun listAppProfileTemplates(): List<String> {
    val shell = getRootShell()
    return shell.newJob().add("${getMySuDaemonPath()} profile list-templates").to(ArrayList(), null)
        .exec().out
}

fun getAppProfileTemplate(id: String): String {
    val shell = getRootShell()
    return shell.newJob().add("${getMySuDaemonPath()} profile get-template '${id}'")
...
fun setAppProfileTemplate(id: String, template: String): Boolean {
    ...
    val cmd = """${getMySuDaemonPath()} profile set-template "$id" '$escapedTemplate'"""
...
fun deleteAppProfileTemplate(id: String): Boolean {
    val shell = getRootShell()
    return shell.newJob().add("${getMySuDaemonPath()} profile delete-template '${id}'")
        .to(ArrayList(), null).exec().isSuccess
}
