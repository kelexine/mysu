package dev.kelexine.mysu.ui.navigation3
...
import dev.kelexine.mysu.ui.util.DownloadService
...
private sealed interface MySuDeepLink {
    data class Action(val moduleId: String) : MySuDeepLink
    data class WebUi(val moduleId: String) : MySuDeepLink
}
...
fun getDisplayName(uri: Uri): String {
    return uri.getFileName(mysuApp) ?: uri.lastPathSegment ?: "Unknown"
}
...
private fun resolveIntent(intent: Intent): PendingAction? {
    ...
    return when (val deepLink = parseValidatedDeepLink(intent.data)) {
        is MySuDeepLink.Action -> PendingAction.ExecuteAction(deepLink.moduleId)
        is MySuDeepLink.WebUi -> PendingAction.OpenWebUI(deepLink.moduleId)
        null -> null
    }
}

private fun parseValidatedDeepLink(uri: Uri?): MySuDeepLink? {
    if (uri?.scheme != SCHEME_MYSU) return null
    ...
    return when (uri.host) {
        HOST_ACTION -> MySuDeepLink.Action(moduleId)
        HOST_WEBUI -> MySuDeepLink.WebUi(moduleId)
        else -> null
    }
}
