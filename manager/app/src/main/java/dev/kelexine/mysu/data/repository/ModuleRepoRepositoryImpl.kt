...
import dev.kelexine.mysu.data.model.Author
import dev.kelexine.mysu.data.model.ReleaseAsset
...
class ModuleRepoRepositoryImpl : ModuleRepoRepository {
    ...
    override suspend fun fetchModules(): Result<List<RepoModule>> = withContext(Dispatchers.IO) {
        runCatching {
            if (!isNetworkAvailable(mysuApp)) {
                throw Exception("Network unavailable")
            }

            val request = Request.Builder().url(MODULES_URL).build()
            mysuApp.okhttpClient.newCall(request).execute().use { response ->
