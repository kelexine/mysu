...
fun getBugreportFile(context: Context): File {
    ...
    val mysuFileSize = File(bugreportDir, "mysu_size.txt")
    ...
    shell.newJob().add("tar -czf ${bootlogFile.absolutePath} -C /data/adb/mysu/log .").exec()
    ...
    shell.newJob().add("du -sh /data/adb/mysu/* > ${mysuFileSize.absolutePath}").exec()
    shell.newJob().add("cp /data/system/packages.list ${appListFile.absolutePath}").exec()
    shell.newJob().add("getprop > ${propFile.absolutePath}").exec()
    shell.newJob().add("cp /data/adb/mysu/.allowlist ${allowListFile.absolutePath}").exec()
    ...
    PrintWriter(FileWriter(buildInfo)).use { pw ->
        ...
        val mysuKernel = Natives.version
        pw.println("MySU: $mysuKernel")
    ...
    val targetFile = File(context.cacheDir, "MySU_bugreport_${current}.tar.gz")
