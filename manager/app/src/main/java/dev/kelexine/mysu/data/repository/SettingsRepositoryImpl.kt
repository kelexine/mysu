...
import dev.kelexine.mysu.magica.BootCompletedReceiver
import dev.kelexine.mysu.ui.UiMode
import dev.kelexine.mysu.ui.screen.modulerepo.RepoSort
import dev.kelexine.mysu.ui.util.execMySud
import dev.kelexine.mysu.ui.util.getFeaturePersistValue
import dev.kelexine.mysu.ui.util.getFeatureStatus
...
fun isSoftRebootPreferred(): Boolean =
    Natives.isLateLoadMode || mysuApp.getSharedPreferences(SETTINGS_PREFS, Context.MODE_PRIVATE)
...
class SettingsRepositoryImpl : SettingsRepository {
    ...
    private val prefs by lazy {
        mysuApp.getSharedPreferences(SETTINGS_PREFS, Context.MODE_PRIVATE)
    }
    ...
    override var autoJailbreak: Boolean
        ...
        set(value) {
            runCatching {
                mysuApp.packageManager.setComponentEnabledSetting(
                    ComponentName(mysuApp, BootCompletedReceiver::class.java),
    ...
    override fun setSulogEnabled(enabled: Boolean): Boolean = execMySud("feature set sulog ${if (enabled) 1 else 0}", true)
    ...
    override fun setAdbRootEnabled(enabled: Boolean): Boolean =
        if (execMySud("feature set adb_root ${if (enabled) 1 else 0}", true)) {
    ...
    override fun execMySudFeatureSave() {
        execMySud("feature save", true)
    }
}
