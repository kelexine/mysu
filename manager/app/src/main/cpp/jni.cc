...
Java_dev_kelexine_mysu_Natives_getVersion(JNIEnv *env, jobject) {
...
extern "C"
JNIEXPORT jint JNICALL
Java_dev_kelexine_mysu_Natives_getKernelUAPIVersion(JNIEnv *env, jobject) {
    return get_kernel_uapi_version();
}

extern "C"
JNIEXPORT jint JNICALL
Java_dev_kelexine_mysu_Natives_getManagerUAPIVersion(JNIEnv *env, jobject) {
    return get_manager_uapi_version();
}

extern "C"
JNIEXPORT jint JNICALL
Java_dev_kelexine_mysu_Natives_getSuperuserCount(JNIEnv *env, jobject) {
...
Java_dev_kelexine_mysu_Natives_isSafeMode(JNIEnv *env, jclass clazz) {
    return is_safe_mode();
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isLkmMode(JNIEnv *env, jclass clazz) {
    return is_lkm_mode();
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isLateLoadMode(JNIEnv *env, jclass clazz) {
    return is_late_load_mode();
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isManager(JNIEnv *env, jclass clazz) {
    return is_manager();
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isPrBuild(JNIEnv *env, jclass clazz) {
    return is_pr_build();
}
...
Java_dev_kelexine_mysu_Natives_getAppProfile(JNIEnv *env, jobject, jstring pkg, jint uid) {
    if (env->GetStringLength(pkg) > MYSU_MAX_PACKAGE_NAME) {
        return nullptr;
    }
    ...
    app_profile profile = {};
    profile.version = MYSU_APP_PROFILE_VER;
    ...
    auto cls = env->FindClass("dev/kelexine/mysu/Natives$Profile");
    ...
    if (allowSu) {
        ...
        if (groupCount > MYSU_MAX_GROUPS) {
            LOGD("kernel group count too large: %d???", groupCount);
            groupCount = MYSU_MAX_GROUPS;
        }
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_setAppProfile(JNIEnv *env, jobject clazz, jobject profile) {
    auto cls = env->FindClass("dev/kelexine/mysu/Natives$Profile");
    ...
    if (env->GetStringLength((jstring) key) > MYSU_MAX_PACKAGE_NAME) {
        return false;
    }
    ...
    app_profile p = {};
    p.version = MYSU_APP_PROFILE_VER;
    ...
    if (allowSu) {
        ...
        if (groups_count > MYSU_MAX_GROUPS) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_uidShouldUmount(JNIEnv *env, jobject thiz, jint uid) {
    return uid_should_umount(uid);
}
extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isSuEnabled(JNIEnv *env, jobject thiz) {
    return is_su_enabled();
}
extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_setSuEnabled(JNIEnv *env, jobject thiz, jboolean enabled) {
    return set_su_enabled(enabled);
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isKernelUmountEnabled(JNIEnv *env, jobject thiz) {
    return is_kernel_umount_enabled();
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_setKernelUmountEnabled(JNIEnv *env, jobject thiz, jboolean enabled) {
    return set_kernel_umount_enabled(enabled);
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_dev_kelexine_mysu_Natives_isSelinuxHideEnabled(JNIEnv *env, jobject thiz) {
    return is_selinux_hide_enabled();
}

extern "C"
JNIEXPORT jint JNICALL
Java_dev_kelexine_mysu_Natives_setSelinuxHideEnabled(JNIEnv *env, jobject thiz, jboolean enabled) {
...
int fork_dont_care_and_exec_mysud(const char *path, const char *pkg) {
    int pid = fork();
    if (pid < 0) {
        PLOGE("fork");
        return pid;
    ...
    execl(path, "mysud", "late-load", "--magica", "5555", "--package-name", pkg, nullptr);
    PLOGE("exec magica");
    _exit(1);
}
...
Java_dev_kelexine_mysu_magica_AppZygotePreload_forkDontCareAndExecMysud(JNIEnv *env, jclass clazz,
                                                                        jstring mysud_path, jstring pkg_name) {
    auto path = env->GetStringUTFChars(mysud_path, nullptr);
    auto pkg = env->GetStringUTFChars(pkg_name, nullptr);
    LOGD("executing magica %s (pkg %s)", path, pkg);
    fork_dont_care_and_exec_mysud(path, pkg);
    env->ReleaseStringUTFChars(mysud_path, path);
