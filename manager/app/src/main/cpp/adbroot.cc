...
// If the user creates mysurc, use mysurc instead of mkshrc
...
int execle(const char *pathname, const char *arg, ...) {
    ...
    bool mysurc_exists = (access("/data/adb/mysu/.mysurc", F_OK) == 0);
    const char *mysu_env_str = "ENV=/data/adb/mysu/.mysurc";
    ...
    if (old_envp != nullptr) {
        for (size_t i = 0; old_envp[i] != nullptr; ++i) {
            ...
            if (mysurc_exists && strncmp(old_envp[i], "ENV=", 4) == 0) {
                continue;
            }
    ...
    if (mysurc_exists) {
        ...
        env_list.push_back(mysu_env_str);
    }
