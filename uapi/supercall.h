#ifndef __MYSU_UAPI_SUPERCALL_H
#define __MYSU_UAPI_SUPERCALL_H
...
static const __u32 MYSU_GET_INFO_FLAG_LKM = (1U << 0);
static const __u32 MYSU_GET_INFO_FLAG_MANAGER = (1U << 1);
 ...
 * MYSU_SEPOLICY_CMD_NORMAL_PERM=4, MYSU_SEPOLICY_CMD_XPERM=5,
 * MYSU_SEPOLICY_CMD_TYPE_STATE=1, MYSU_SEPOLICY_CMD_TYPE=2,
 * MYSU_SEPOLICY_CMD_TYPE_ATTR=2, MYSU_SEPOLICY_CMD_ATTR=1,
 * MYSU_SEPOLICY_CMD_TYPE_TRANSITION=5, MYSU_SEPOLICY_CMD_TYPE_CHANGE=4,
 * MYSU_SEPOLICY_CMD_GENFSCON=3.
 */
...
static const __u32 MYSU_IOCTL_GET_INFO = _IOR('K', 2, struct mysu_get_info_cmd);
/* deprecated */
static const __u32 MYSU_IOCTL_GET_INFO_LEGACY = _IOC(_IOC_READ, 'K', 2, 0);
static const __u32 MYSU_IOCTL_REPORT_EVENT = _IOC(_IOC_WRITE, 'K', 3, 0);
static const __u32 MYSU_IOCTL_SET_SEPOLICY = _IOC(_IOC_READ | _IOC_WRITE, 'K', 4, 0);
static const __u32 MYSU_IOCTL_CHECK_SAFEMODE = _IOC(_IOC_READ, 'K', 5, 0);
/* deprecated */
static const __u32 MYSU_IOCTL_GET_ALLOW_LIST = _IOC(_IOC_READ | _IOC_WRITE, 'K', 6, 0);
/* deprecated */
static const __u32 MYSU_IOCTL_GET_DENY_LIST = _IOC(_IOC_READ | _IOC_WRITE, 'K', 7, 0);
static const __u32 MYSU_IOCTL_NEW_GET_ALLOW_LIST = _IOWR('K', 6, struct mysu_new_get_allow_list_cmd);
static const __u32 MYSU_IOCTL_NEW_GET_DENY_LIST = _IOWR('K', 7, struct mysu_new_get_allow_list_cmd);
static const __u32 MYSU_IOCTL_UID_GRANTED_ROOT = _IOC(_IOC_READ | _IOC_WRITE, 'K', 8, 0);
static const __u32 MYSU_IOCTL_UID_SHOULD_UMOUNT = _IOC(_IOC_READ | _IOC_WRITE, 'K', 9, 0);
static const __u32 MYSU_IOCTL_GET_MANAGER_APPID = _IOC(_IOC_READ, 'K', 10, 0);
static const __u32 MYSU_IOCTL_GET_APP_PROFILE = _IOC(_IOC_READ | _IOC_WRITE, 'K', 11, 0);
static const __u32 MYSU_IOCTL_SET_APP_PROFILE = _IOC(_IOC_WRITE, 'K', 12, 0);
static const __u32 MYSU_IOCTL_GET_FEATURE = _IOC(_IOC_READ | _IOC_WRITE, 'K', 13, 0);
static const __u32 MYSU_IOCTL_SET_FEATURE = _IOC(_IOC_WRITE, 'K', 14, 0);
static const __u32 MYSU_IOCTL_GET_WRAPPER_FD = _IOC(_IOC_WRITE, 'K', 15, 0);
static const __u32 MYSU_IOCTL_MANAGE_MARK = _IOC(_IOC_READ | _IOC_WRITE, 'K', 16, 0);
static const __u32 MYSU_IOCTL_NUKE_EXT4_SYSFS = _IOC(_IOC_WRITE, 'K', 17, 0);
static const __u32 MYSU_IOCTL_ADD_TRY_UMOUNT = _IOC(_IOC_WRITE, 'K', 18, 0);
static const __u32 MYSU_IOCTL_SET_INIT_PGRP = _IO('K', 19);
static const __u32 MYSU_IOCTL_GET_SULOG_FD = _IOW('K', 20, struct mysu_get_sulog_fd_cmd);
