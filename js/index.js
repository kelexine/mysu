...
export function exec(command, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      mysu.exec(command, JSON.stringify(options), callbackFuncName);
    } catch (error) {
      reject(error);
      cleanup(callbackFuncName);
    }
...
export function fullScreen(isFullScreen) {
  mysu.fullScreen(isFullScreen);
}

export function enableEdgeToEdge(enable) {
  mysu.enableEdgeToEdge(enable);
}
...
export function getPackagesInfo(packages) {
  try {
    ...
    return JSON.parse(mysu.getPackagesInfo(packages));
  } catch (error) {
    return [];
  }
}
