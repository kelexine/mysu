...
static long mysu_sys_setns(int fd, int flags)
{
    struct pt_regs regs;
...
static void mysu_mnt_ns_global(void)
{
...
try_setns:
    ...
    struct file *ns_file = dentry_open(&ns_path, O_RDONLY, mysu_cred);
    ...
    ret = mysu_sys_setns(fd, CLONE_NEWNS);
...
static void mysu_mnt_ns_individual(void)
{
...
{
    // inherit mode
    if (ns_mode == MYSU_NS_INHERITED) {
        // do nothing
        return;
    }

    if (ns_mode != MYSU_NS_GLOBAL && ns_mode != MYSU_NS_INDIVIDUAL) {
        pr_warn("pid: %d ,unknown mount namespace mode: %d\n", current->pid, ns_mode);
        return;
    }

    const struct cred *old_cred = override_creds(mysu_cred);
    if (ns_mode == MYSU_NS_GLOBAL) {
        mysu_mnt_ns_global();
    } else {
        mysu_mnt_ns_individual();
    }
