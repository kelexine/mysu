...
static inline bool mysu_symbol_has_suffix(const char *name, size_t name_len, const char *suffix, size_t suffix_len)
{
...
#endif
{
    struct mysu_lookup_symbol_ctx *ctx = data;
    size_t name_len;
...
#if !USE_KCFI
    if (mysu_symbol_has_suffix(name, name_len, cfi_suffix, cfi_suffix_len)) {
        ctx->match = (void *)addr;
...
{
    struct mysu_lookup_symbol_ctx ctx = {
...
void __init mysu_init_symbol_resolver()
{
#if !ALWAYS_HAVE_ON_EACH_SYMBOL
