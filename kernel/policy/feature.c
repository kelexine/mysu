...
static const struct mysu_feature_handler *feature_handlers[MYSU_FEATURE_MAX];

static DEFINE_MUTEX(feature_mutex);

int __init mysu_register_feature_handler(const struct mysu_feature_handler *handler)
{
    if (!handler) {
    ...
    if (handler->feature_id >= MYSU_FEATURE_MAX) {
...
int mysu_unregister_feature_handler(u32 feature_id)
{
    ...
    if (feature_id >= MYSU_FEATURE_MAX) {
        pr_err("feature: invalid feature_id %u\n", feature_id);
        return -EINVAL;
    }
...
{
    int ret = 0;
    const struct mysu_feature_handler *handler;
...
void __init mysu_feature_init(void)
{
    int i;

    for (i = 0; i < MYSU_FEATURE_MAX; i++) {
...
void __exit mysu_feature_exit(void)
{
    int i;
