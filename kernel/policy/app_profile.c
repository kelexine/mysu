...
{
    if (profile->groups_count > MYSU_MAX_GROUPS) {
...
int escape_with_root_profile(void)
{
    ...
    if (test_thread_flag(TIF_MYSU_DISABLE_ESCAPE_WITH_ROOT)) {
        pr_warn("TIF_MYSU_DISABLE_ESCAPE_WITH_ROOT found, don't escape!\n");
        goto out_abort_creds;
    }

    profile = mysu_get_root_profile(cred->uid.val);
    ...
    if (profile->flags & FLAG_MYSU_NO_NEW_PRIVS) {
