...
static void __init init_default_profiles()
{
    ...
    default_root_profile.namespaces = MYSU_NS_INHERITED;
    strcpy(default_root_profile.selinux_domain, MYSU_DEFAULT_SELINUX_DOMAIN);
...
static bool profile_valid(struct app_profile *profile)
{
    ...
    if (profile->version != MYSU_APP_PROFILE_VER) {
        pr_info("Unsupported profile version: %d\n", profile->version);
        return false;
    }

    if (profile->allow_su) {
#ifndef CONFIG_MYSU_DISABLE_POLICY
        if (profile->rp_config.profile.groups_count > MYSU_MAX_GROUPS) {
...
int mysu_set_app_profile(struct app_profile *profile)
{
    ...
    if (unlikely(profile->curr_uid == MYSU_APP_PROFILE_PRESERVE_UID && strcmp(profile->key, "$") != 0)) {
        return -EINVAL;
    }
    ...
    hash_for_each_possible (allow_list, p, list, profile->curr_uid) {
        if (profile->curr_uid == p->profile.curr_uid) {
            if (strcmp(profile->key, p->profile.key) != 0) {
                pr_warn("mysu_set_app_profile: key changed: uid=%d orig=%s new=%s\n", profile->curr_uid, p->profile.key,
                        profile->key);
            }
    ...
    np = (struct perm_data *)kzalloc(sizeof(struct perm_data), GFP_KERNEL);
    if (!np) {
        pr_err("mysu_set_app_profile alloc failed\n");
        result = -ENOMEM;
        goto out_unlock;
    }
...
out:
    ...
    if (unlikely(profile->curr_uid == MYSU_APP_PROFILE_PRESERVE_UID)) {
...
void mysu_put_app_profile(struct app_profile *profile)
{
...
void mysu_put_root_profile(struct root_profile *profile)
{
...
static void migrate_profile(u32 version, struct app_profile *profile)
{
    ...
    case 2:
        if (profile->allow_su) {
            ...
            if (strncmp(domain, "u:r:su:s0", domain_len) == 0) {
                strscpy_pad(domain, MYSU_DEFAULT_SELINUX_DOMAIN, domain_len);
    ...
    case 3:
        if (profile->allow_su) {
            profile->rp_config.profile.flags = FLAG_MYSU_NO_NEW_PRIVS;
        }
        break;
    }
...
void mysu_load_allow_list()
{
    ...
    if (version < 2 || version > MYSU_APP_PROFILE_VER) {
    ...
    app_profile_size = version < MYSU_APP_PROFILE_VER ? kAppProfileSizePreV4 : sizeof(struct app_profile);
    ...
    while (true) {
        ...
        mysu_set_app_profile(&profile);
    }
    mysu_show_allow_list();
    filp_close(fp, 0);
    if (version < MYSU_APP_PROFILE_VER)
...
void mysu_prune_allowlist(bool (*is_uid_valid)(uid_t, char *, void *), void *data)
{
    struct perm_data *np = NULL;
    struct hlist_node *tmp;
    int i;

    if (!mysu_boot_completed) {
    ...
    hash_for_each_safe (allow_list, i, tmp, np, list) {
        ...
        bool is_preserved_uid = uid == MYSU_APP_PROFILE_PRESERVE_UID;
...
void __init mysu_allowlist_init(void)
{
    init_default_profiles();
}

void __exit mysu_allowlist_exit(void)
{
    struct perm_data *np = NULL;
    struct hlist_node *tmp;
    int i;
