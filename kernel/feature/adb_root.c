...
DEFINE_STATIC_KEY_FALSE(mysu_adb_root);
...
{
    static const char kLibAdbRoot[] = "/data/adb/mysu/lib/libadbroot.so";
    struct path path;
    long ret = kern_path(kLibAdbRoot, 0, &path);
    if (ret < 0) {
        if (ret == -ENOENT) {
            pr_err("libadbroot.so not exists, skip adb root. Please run `mysud install`\n");
            ret = 0;
        } else {
...
{
    static const char kLdPreload[] = "LD_PRELOAD=/data/adb/mysu/lib/libadbroot.so";
    static const char kLdLibraryPath[] = "LD_LIBRARY_PATH=/data/adb/mysu/lib";
...
static long do_mysu_adb_root_handle_execve(const char __user *filename_user, struct pt_regs *regs, unsigned long *envp_p)
{
...
{
    if (static_branch_unlikely(&mysu_adb_root)) {
        return do_mysu_adb_root_handle_execve((const char __user *)PT_REGS_PARM1(regs), regs,
...
{
    if (static_branch_unlikely(&mysu_adb_root)) {
        return do_mysu_adb_root_handle_execve((const char __user *)PT_REGS_PARM2(regs), regs,
...
{
    *value = static_key_enabled(&mysu_adb_root) ? 1 : 0;
    return 0;
}
...
static int kernel_adb_root_feature_set(u64 value)
{
    bool enable = value != 0;
    if (enable) {
        static_key_enable(&mysu_adb_root.key);
    } else {
        static_key_disable(&mysu_adb_root.key);
    }
    pr_info("adb_root: set to %d\n", enable);
    return 0;
}

static const struct mysu_feature_handler mysu_adb_root_handler = {
    .feature_id = MYSU_FEATURE_ADB_ROOT,
    .name = "adb_root",
    .get_handler = kernel_adb_root_feature_get,
    .set_handler = kernel_adb_root_feature_set,
};

void __init mysu_adb_root_init(void)
{
    if (mysu_register_feature_handler(&mysu_adb_root_handler)) {
        pr_err("Failed to register adb_root feature handler\n");
    }
}

void __exit mysu_adb_root_exit(void)
{
    mysu_unregister_feature_handler(MYSU_FEATURE_ADB_ROOT);
}
