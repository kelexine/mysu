...
struct mysu_lsm_hook selinux_setprocattr_hook = MYSU_LSM_HOOK_INIT(setprocattr, "selinux_setprocattr", my_setprocattr, 0);
...
static void initialize_fake_status()
{
    ...
    if (mysu_late_loaded && !new_status->enforcing) {
...
static void mysu_selinux_hide_unhook();
static int mysu_selinux_hide_enable()
{
    int ret;
    ...
    ret = mysu_patch_text(context_write, &my, sizeof(my), MYSU_PATCH_TEXT_FLUSH_DCACHE);
    if (ret) {
    ...
    ret = mysu_patch_text(access_write, &my, sizeof(my), MYSU_PATCH_TEXT_FLUSH_DCACHE);
    if (ret) {
        pr_err("selinux_hide: init: patch_text access_write err: %d\n", ret);
        goto unhook;
    }

    ret = mysu_lsm_hook(&selinux_setprocattr_hook);
    if (ret) {
...
static void mysu_selinux_hide_unhook()
{
    ...
    if (orig_context_write) {
        ret =
            mysu_patch_text(context_write, &orig_context_write, sizeof(orig_context_write), MYSU_PATCH_TEXT_FLUSH_DCACHE);
    ...
    if (orig_access_write) {
        ret = mysu_patch_text(access_write, &orig_access_write, sizeof(orig_access_write), MYSU_PATCH_TEXT_FLUSH_DCACHE);
    ...
    if (sel_open_handle_status_slot && orig_sel_open_handle_status) {
        ret = mysu_patch_text(sel_open_handle_status_slot, &orig_sel_open_handle_status,
                             sizeof(orig_sel_open_handle_status), MYSU_PATCH_TEXT_FLUSH_DCACHE);
        orig_sel_open_handle_status = NULL;
        if (ret) {
            pr_err("selinux_hide: exit: patch_text sel_open_handle_status err: %d\n", ret);
        }
    }
    mysu_lsm_unhook(&selinux_setprocattr_hook);
}

static void mysu_selinux_hide_disable()
{
...
{
    *value = mysu_selinux_hide_enabled ? 1 : 0;
    return 0;
}
...
static int selinux_hide_feature_set(u64 value)
{
    ...
    mysu_selinux_hide_enabled = enable;
    if (enable) {
        if (!mysu_selinux_hide_running) {
            ret = mysu_selinux_hide_enable();
            if (!ret) {
                mysu_selinux_hide_running = true;
            }
        }
    } else {
        if (mysu_selinux_hide_running) {
            mysu_selinux_hide_disable();
            mysu_selinux_hide_running = false;
        }
    }
...
static const struct mysu_feature_handler selinux_hide_handler = {
    .feature_id = MYSU_FEATURE_SELINUX_HIDE,
    .name = "selinux_hide",
...
static void hook_selinux_status_open()
{
    ...
    int ret = mysu_patch_text(sel_open_handle_status_slot, &new_fn, sizeof(new_fn), MYSU_PATCH_TEXT_FLUSH_DCACHE);
    if (ret) {
...
void __init mysu_selinux_hide_init()
{
    if (mysu_register_feature_handler(&selinux_hide_handler)) {
...
void __exit mysu_selinux_hide_exit()
{
    mutex_lock(&selinux_hide_mutex);
    if (mysu_selinux_hide_running) {
        mysu_selinux_hide_disable();
