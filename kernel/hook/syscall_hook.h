#ifndef __MYSU_H_SYSCALL_HOOK
#define __MYSU_H_SYSCALL_HOOK
...
extern syscall_fn_t *mysu_syscall_table;

// Dispatcher slot number in syscall table
extern int mysu_dispatcher_nr;

// Syscall hook handler type.
// orig_nr: the original syscall number before redirection
// regs: the original pt_regs from userspace
// Handler is responsible for calling mysu_syscall_table[orig_nr](regs) if needed.
typedef long (*mysu_syscall_hook_fn)(int orig_nr, const struct pt_regs *regs);
...
// (e.g. mysud unhooking __NR_read after init.rc injection is done).
void mysu_syscall_table_unhook(int nr);

void mysu_syscall_hook_init(void);
void mysu_syscall_hook_exit(void);
