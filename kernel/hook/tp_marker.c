...
int mysu_tp_marker_reg_count(void)
{
    return tracepoint_reg_count;
}

void mysu_tp_marker_lock(unsigned long *flags)
{
    spin_lock_irqsave(&tracepoint_reg_lock, *flags);
}

void mysu_tp_marker_unlock(unsigned long *flags)
{
    spin_unlock_irqrestore(&tracepoint_reg_lock, *flags);
}

void mysu_tp_marker_inc_reg_count(void)
{
    tracepoint_reg_count++;
}

void mysu_tp_marker_dec_reg_count(void)
{
    tracepoint_reg_count--;
}

void mysu_clear_task_tracepoint_flag_if_needed(struct task_struct *t)
{
    unsigned long flags;
    spin_lock_irqsave(&tracepoint_reg_lock, flags);
    if (tracepoint_reg_count <= 1) {
        mysu_clear_task_tracepoint_flag(t);
    }
    spin_unlock_irqrestore(&tracepoint_reg_lock, flags);
}
...
static void handle_process_mark(bool mark)
{
    ...
    for_each_process_thread (p, t) {
        if (mark)
            mysu_set_task_tracepoint_flag(t);
        else
            mysu_clear_task_tracepoint_flag(t);
    }
...
void mysu_mark_running_process_locked(void)
{
    ...
    for_each_process_thread (p, t) {
        ...
        bool mysu_root_process = uid == 0 && is_task_mysu_domain(cred);
        ...
        if (mysu_root_process || is_zygote_process || is_shell || is_init || mysu_is_allow_uid(uid)) {
            mysu_set_task_tracepoint_flag(t);
...
void mysu_mark_running_process(void)
{
    unsigned long flags;
    spin_lock_irqsave(&tracepoint_reg_lock, flags);
    if (tracepoint_reg_count <= 1) {
...
int mysu_get_task_mark(pid_t pid)
{
...
int mysu_set_task_mark(pid_t pid, bool mark)
{
    struct task_struct *task;
    int ret = -ESRCH;
    ...
    if (task) {
        ...
        rcu_read_unlock();
        if (mark) {
            mysu_set_task_tracepoint_flag(task);
            pr_info("tp_marker: marked task pid=%d comm=%s\n", pid, task->comm);
        } else {
            mysu_clear_task_tracepoint_flag(task);
