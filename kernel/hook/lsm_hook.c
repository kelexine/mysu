...
#endif
struct mysu_lsm_hook_entry {
    struct mysu_lsm_hook *hook;
};

static DEFINE_MUTEX(mysu_lsm_hook_lock);
static struct mysu_lsm_hook_entry mysu_lsm_hook_entries[16];
static int mysu_lsm_hook_count;

static bool mysu_lsm_hook_is_tracked(struct mysu_lsm_hook *hook)
{
    int i;

    for (i = 0; i < mysu_lsm_hook_count; i++) {
        if (mysu_lsm_hook_entries[i].hook == hook)
            return true;
    }
...
static int mysu_lsm_hook_track(struct mysu_lsm_hook *hook)
{
    if (mysu_lsm_hook_is_tracked(hook))
        return 0;

    if (mysu_lsm_hook_count >= ARRAY_SIZE(mysu_lsm_hook_entries)) {
        pr_err("lsm_hook: tracking table full, cannot record %s\n", hook->head_name ?: "unknown");
        return -ENOSPC;
    }

    mysu_lsm_hook_entries[mysu_lsm_hook_count++].hook = hook;
    return 0;
}

static void mysu_lsm_hook_untrack(struct mysu_lsm_hook *hook)
{
    int i;
    ...
    for (i = 0; i < mysu_lsm_hook_count; i++) {
        ...
        mysu_lsm_hook_entries[i] = mysu_lsm_hook_entries[--mysu_lsm_hook_count];
        return;
    }
}

static int mysu_lsm_hook_patch_slot(void **slot, void *value)
{
    void *patched = value;
    int ret;

    ret = mysu_patch_text(slot, &patched, sizeof(patched), MYSU_PATCH_TEXT_FLUSH_DCACHE);
    if (!ret)
        smp_wmb();
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
typedef void (*mysu_static_call_update_t)(struct static_call_key *key, void *tramp, void *func);

static int mysu_lsm_hook_update_scall(struct lsm_static_call *scall, void *value)
{
...
int mysu_lsm_hook(struct mysu_lsm_hook *hook)
{
    int ret = 0;
    ...
    if (!target)
        target = mysu_resolve_symbol_for_functable_hook(target_name);
    if (!target) {
    ...
    for (i = 0; i < scalls_count; i++) {
        ...
        int j;
        for (j = 0; j < mysu_lsm_hook_count; j++) {
            if (mysu_lsm_hook_entries[j].hook->replacement == current_origin) {
                current_origin = mysu_lsm_hook_entries[j].hook->original;
                break;
            }
        }
    ...
    ret = mysu_lsm_hook_track(hook);
    if (ret) {
        pr_err("lsm_hook: too many hooks to track: %d\n", ret);
        goto out_unlock;
    }

    if (mysu_lsm_hook_patch_slot(selected_slot, hook->replacement)) {
        pr_err("lsm_hook: failed to patch %s\n", hook->head_name ?: "unknown");
        ret = -EFAULT;
        goto out_untrack;
    }

    if (mysu_lsm_hook_update_scall(selected_scall, hook->replacement)) {
        if (mysu_lsm_hook_patch_slot(selected_slot, selected_origin)) {
    ...
    if (selected_origin) {
        ...
        ret = mysu_lsm_hook_patch_slot(selected_slot, hook->replacement);
    } else {
        pr_info("patch head->first\n");
        ret = mysu_lsm_hook_patch_slot(selected_slot, &hook->list);
    }
    ...
    goto out_unlock;
out_untrack:
    mysu_lsm_hook_untrack(hook);

out_unlock:
    mutex_unlock(&mysu_lsm_hook_lock);
    return ret;
}

void mysu_lsm_unhook(struct mysu_lsm_hook *hook)
{
    void **slot;
    mutex_lock(&mysu_lsm_hook_lock);
...
#endif
    if (mysu_lsm_hook_patch_slot(slot, hook->original)) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
    if (mysu_lsm_hook_update_scall(hook->scall, hook->original)) {
        if (mysu_lsm_hook_patch_slot(slot, hook->replacement))
...
int mysu_register_lsm_hook(struct mysu_lsm_hook *hook)
{
    return mysu_lsm_hook(hook);
}

void mysu_unregister_lsm_hook(struct mysu_lsm_hook *hook)
{
    mysu_lsm_unhook(hook);
}

void __init mysu_lsm_hook_init(void)
{
    pr_info("lsm_hook: init, tracked hooks=%d\n", READ_ONCE(mysu_lsm_hook_count));
}

void __exit mysu_lsm_hook_exit(void)
{
    struct mysu_lsm_hook *hooks[ARRAY_SIZE(mysu_lsm_hook_entries)];
    int count;
    int i;

    mutex_lock(&mysu_lsm_hook_lock);
    count = mysu_lsm_hook_count;
    for (i = 0; i < count; i++)
        hooks[i] = mysu_lsm_hook_entries[i].hook;
    mutex_unlock(&mysu_lsm_hook_lock);

    for (i = count - 1; i >= 0; i--)
        mysu_lsm_unhook(hooks[i]);
}
