...
sys_call_ptr_t *mysu_syscall_table = NULL;
int mysu_dispatcher_nr = -1;
...
static mysu_syscall_hook_fn syscall_hooks[__NR_syscalls];
...
static int patch_syscall_table(int nr, sys_call_ptr_t fn)
{
    ...
    pr_info("patch syscall %d, 0x%lx -> 0x%lx\n", nr, (unsigned long)READ_ONCE(mysu_syscall_table[nr]),
            (unsigned long)fn);

    if (mysu_patch_text(&mysu_syscall_table[nr], &fn, sizeof(fn), MYSU_PATCH_TEXT_FLUSH_DCACHE)) {
...
void mysu_syscall_table_hook(int nr, sys_call_ptr_t fn, sys_call_ptr_t *old)
{
    if (mysu_syscall_table == NULL)
        return;
    if (nr < 0 || nr >= __NR_syscalls) {
    ...
    sys_call_ptr_t orig = READ_ONCE(mysu_syscall_table[nr]);
    if (old)
        *old = orig;
...
void mysu_syscall_table_unhook(int nr)
{
    int i;
...
static int mysu_find_ni_syscall_slots(int *out_slots, int max_slots)
{
    unsigned long ni_syscall;
    int i, count = 0;

    if (!mysu_syscall_table || max_slots <= 0)
        return 0;

    ni_syscall = (unsigned long)mysu_resolve_symbol_for_functable_hook("__x64_sys_ni_syscall");
    ...
    for (i = 0; i < __NR_syscalls && count < max_slots; i++) {
        if ((unsigned long)mysu_syscall_table[i] == ni_syscall) {
            out_slots[count++] = i;
...
static long __nocfi mysu_syscall_dispatcher(const struct pt_regs *regs)
{
    if (regs->orig_ax != mysu_dispatcher_nr)
        return -ENOSYS;
    ...
    if (likely(orig_nr >= 0 && orig_nr < __NR_syscalls)) {
        mysu_syscall_hook_fn fn = READ_ONCE(syscall_hooks[orig_nr]);
        if (likely(fn))
...
int mysu_register_syscall_hook(int nr, mysu_syscall_hook_fn fn)
{
    if (nr < 0 || nr >= __NR_syscalls)
        return -EINVAL;
...
void mysu_unregister_syscall_hook(int nr)
{
    if (nr < 0 || nr >= __NR_syscalls)
        return;
...
bool mysu_has_syscall_hook(int nr)
{
    if (nr < 0 || nr >= __NR_syscalls)
        return false;
...
#ifdef CONFIG_MYSU_X86_PATCH_SYSCALL_DISPATCHER
...
{
    return mysu_syscall_table[nr](regs);
}
...
static __always_inline bool my_do_syscall_x64(struct pt_regs *regs, int nr)
{
    ...
    if (likely(unr < NR_syscalls)) {
        ...
        regs->ax = mysu_syscall_table[unr](regs);
        return true;
    }
    return false;
}
    ...
    *patch_addr = (void *)find_kernel_symbol_exact(sym);
    ...
    if (*patch_addr) {
        ...
        *((void **)(buf + 6)) = target;
        ...
        int ret = mysu_patch_text(*patch_addr, buf, sizeof(buf), MYSU_PATCH_TEXT_FLUSH_ICACHE);
        if (ret) {
...
void __init __nocfi mysu_syscall_hook_init(void)
{
    int ni_slot;

    memset(syscall_hooks, 0, sizeof(syscall_hooks));

    mysu_syscall_table = (sys_call_ptr_t *)mysu_resolve_symbol_for_functable_hook("sys_call_table");
    ...
    if (mysu_find_ni_syscall_slots(&ni_slot, 1) < 1) {
        pr_err("failed to find ni_syscall slot for dispatcher\n");
        return;
    }

    mysu_dispatcher_nr = ni_slot;
    mysu_syscall_table_hook(mysu_dispatcher_nr, (sys_call_ptr_t)mysu_syscall_dispatcher, NULL);
    pr_info("dispatcher installed at slot %d\n", mysu_dispatcher_nr);
}

void __exit mysu_syscall_hook_exit(void)
{
    int i;
    ...
    if (x64_sys_call_patch_addr) {
        ret = mysu_patch_text((void *)x64_sys_call_patch_addr, x64_sys_call_patch_orig_insn,
                             sizeof(x64_sys_call_patch_orig_insn), MYSU_PATCH_TEXT_FLUSH_ICACHE);
        if (ret) {
    ...
    if (do_syscall_64_patch_addr) {
        ret = mysu_patch_text((void *)do_syscall_64_patch_addr, do_syscall_64_orig_insn, sizeof(do_syscall_64_orig_insn),
                             MYSU_PATCH_TEXT_FLUSH_ICACHE);
        if (ret) {
            pr_err("restore x64_sys_call err: %d\n", ret);
        }
    }
    ...
    for (i = 0; i < hooked_count; i++) {
        ...
        if (mysu_patch_text(&mysu_syscall_table[nr], &orig, sizeof(orig), MYSU_PATCH_TEXT_FLUSH_DCACHE)) {
...
clear_state:
    ...
    // (done by mysu_syscall_hook_manager_exit before calling us), so no new
