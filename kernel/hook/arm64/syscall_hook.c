...
syscall_fn_t *mysu_syscall_table = NULL;
...
void mysu_syscall_table_hook(int nr, syscall_fn_t fn, syscall_fn_t *old)
{
    ...
    syscall_fn_t orig = READ_ONCE(mysu_syscall_table[nr]);
    if (old)
        *old = orig;
...
static int __init mysu_find_ni_syscall_slots(int *out_slots, int max_slots)
{
...
{
    if (regs->syscallno != mysu_dispatcher_nr)
        return -ENOSYS;
