...
uid_t mysu_manager_appid = MYSU_INVALID_APPID;
...
struct uid_data {
    ...
    u32 uid;
    char package[MYSU_MAX_PACKAGE_NAME];
};
...
static void crown_manager(const char *apk, struct list_head *uid_data)
{
    ...
    list_for_each_entry (np, list, list) {
        if (strncmp(np->package, pkg, MYSU_MAX_PACKAGE_NAME) == 0) {
            pr_info("Crowning manager: %s(uid=%d)\n", pkg, np->uid);
            mysu_set_manager_appid(np->uid);
            break;
        }
    }
}
...
static bool is_uid_exist(uid_t uid, char *package, void *data)
{
    ...
    bool exist = false;
    list_for_each_entry (np, list, list) {
        if (np->uid == uid % PER_USER_RANGE && strncmp(np->package, package, MYSU_MAX_PACKAGE_NAME) == 0) {
            exist = true;
            break;
        }
    }
...
void track_throne(bool prune_only)
{
    ...
    loff_t pos = 0;
    loff_t line_start = 0;
    char buf[MYSU_MAX_PACKAGE_NAME];
    for (;;) {
    ...
    list_for_each_entry (np, &uid_list, list) {
        if (np->uid == mysu_get_manager_appid()) {
            manager_exist = true;
            break;
        }
    }
...
prune:
    ...
    mysu_prune_allowlist(is_uid_exist, &uid_list);
out:
    // free uid_list
...
void __exit mysu_throne_tracker_exit()
{
    struct apk_path_hash *pos, *n;
