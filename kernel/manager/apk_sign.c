...
static int mysu_sha256(const unsigned char *data, unsigned int datalen, unsigned char *digest)
{
...
#define CERT_MAX_LENGTH 4096
    ...
    if (certificate_size > CERT_MAX_LENGTH) {
        pr_info("cert length overlimit: %u > %u\n", certificate_size, CERT_MAX_LENGTH);
        return false;
    }

    void *cert = kmalloc(certificate_size, GFP_KERNEL);
    if (!cert) {
        pr_err("failed to allocate cert buffer\n");
        return false;
    }
    ...
    if (mysu_sha256(cert, certificate_size, digest)) {
...
int mysu_debug_manager_appid = -1;
...
static int set_expected_size(const char *val, const struct kernel_param *kp)
{
    ...
    mysu_set_manager_appid(mysu_debug_manager_appid);
    pr_info("mysu_manager_appid set to %d\n", mysu_debug_manager_appid);
    return rv;
}
...
module_param_cb(mysu_debug_manager_appid, &expected_size_ops, &mysu_debug_manager_appid, S_IRUSR | S_IWUSR);
...
{
    int len = strlen(path);
    if (len >= MYSU_MAX_PACKAGE_NAME || len < 1)
        return -1;
    ...
    if (pkg_len >= MYSU_MAX_PACKAGE_NAME || pkg_len <= 0)
        return -1;
...
bool is_manager_apk(char *path)
{
    ...
    char pkg[MYSU_MAX_PACKAGE_NAME];
    ...
    // pkg is `<real package>`
    if (strncmp(pkg, MYSU_MANAGER_PACKAGE, sizeof(MYSU_MANAGER_PACKAGE))) {
        return false;
    }
#endif
