...
static int do_grant_root(void __user *arg)
{
    ...
    mysu_sulog_emit_grant_root(ret, audit_uid, audit_euid, GFP_KERNEL);
...
static int do_get_info(void __user *arg)
{
    ...
    if (is_manager()) {
        cmd.flags |= MYSU_GET_INFO_FLAG_MANAGER;
    }
    if (mysu_late_loaded) {
        cmd.flags |= MYSU_GET_INFO_FLAG_LATE_LOAD;
    }
...
{
    struct mysu_report_event_cmd cmd;
...
{
    struct mysu_set_sepolicy_cmd cmd;
...
static int do_check_safemode(void __user *arg)
{
    ...
    cmd.in_safe_mode = mysu_is_safe_mode();
...
{
    struct mysu_new_get_allow_list_cmd cmd;
    int *arr = NULL;
    int err = 0;
    ...
    bool success = mysu_get_allow_list(arr, cmd.count, &cmd.count, &cmd.total_count, allow);
    ...
    if (cmd.count && copy_to_user(&((struct mysu_new_get_allow_list_cmd *)arg)->uids, arr, sizeof(int) * cmd.count)) {
...
static int do_get_allow_list_common(void __user *arg, bool allow)
{
    ...
    bool success = mysu_get_allow_list(arr, kSize, &count, NULL, allow);
    ...
    if (copy_to_user(arg + offsetof(struct mysu_get_allow_list_cmd, count), &out_count, sizeof(u32))) {
...
{
    struct mysu_uid_granted_root_cmd cmd;

    if (copy_from_user(&cmd, arg, sizeof(cmd))) {
        return -EFAULT;
    }

    cmd.granted = mysu_is_allow_uid_for_current(cmd.uid);
...
static int do_uid_should_umount(void __user *arg)
{
    ...
    cmd.should_umount = mysu_uid_should_umount(cmd.uid);
...
{
    struct mysu_get_manager_appid_cmd cmd;

    cmd.appid = mysu_get_manager_appid();
...
{
#ifdef CONFIG_MYSU_DISABLE_POLICY
    return -EOPNOTSUPP;
#endif
    uid_t uid;
    ...
    if (copy_from_user(&uid, (char __user *)arg + offsetof(struct mysu_get_app_profile_cmd, profile.curr_uid),
                       sizeof(uid_t))) {
    ...
    if (!profile) {
        ...
        if (copy_to_user((char __user *)arg + offsetof(struct mysu_get_app_profile_cmd, profile), profile,
...
static int do_set_app_profile(void __user *arg)
{
    ...
    ret = mysu_set_app_profile(&cmd.profile);
    if (!ret) {
        mysu_persistent_allow_list();
        mysu_mark_running_process();
    }
    return ret;
}
...
static int do_get_feature(void __user *arg)
{
    ...
    ret = mysu_get_feature(cmd.feature_id, &cmd.value, &supported);
...
static int do_set_feature(void __user *arg)
{
    ...
    ret = mysu_set_feature(cmd.feature_id, cmd.value);
    if (ret) {
...
static int do_get_wrapper_fd(void __user *arg)
{
    ...
    struct mysu_get_wrapper_fd_cmd cmd;
    if (copy_from_user(&cmd, arg, sizeof(cmd))) {
        pr_err("get_wrapper_fd: copy_from_user failed\n");
        return -EFAULT;
    }

    return mysu_install_file_wrapper(cmd.fd);
}

static int do_manage_mark(void __user *arg)
{
    struct mysu_manage_mark_cmd cmd;
    int ret = 0;
    ...
    case MYSU_MARK_GET: {
        ...
        ret = mysu_get_task_mark(cmd.pid);
        if (ret < 0) {
    ...
    case MYSU_MARK_MARK: {
        if (cmd.pid == 0) {
            mysu_mark_all_process();
        } else {
            ret = mysu_set_task_mark(cmd.pid, true);
            if (ret < 0) {
    ...
    case MYSU_MARK_UNMARK: {
        if (cmd.pid == 0) {
            mysu_unmark_all_process();
        } else {
            ret = mysu_set_task_mark(cmd.pid, false);
            if (ret < 0) {
...
{
    struct mysu_nuke_ext4_sysfs_cmd cmd;
    char mnt[256];
    long ret;
...
static int add_try_umount(void __user *arg)
{
    ...
    struct mysu_add_try_umount_cmd cmd;
    char buf[256] = { 0 };
...
{
    struct mysu_get_sulog_fd_cmd cmd;
...
long mysu_supercall_handle_ioctl(unsigned int cmd, void __user *argp)
{
    ...
    pr_info("mysu ioctl: cmd=0x%x from uid=%d\n", cmd, current_uid().val);
#endif

    for (i = 0; mysu_ioctl_handlers[i].handler; i++) {
        if (cmd == mysu_ioctl_handlers[i].cmd) {
            // Check permission first
            if (mysu_ioctl_handlers[i].perm_check && !mysu_ioctl_handlers[i].perm_check()) {
                pr_warn("mysu ioctl: permission denied for cmd=0x%x uid=%d\n", cmd, current_uid().val);
                return -EPERM;
            }
            // Execute handler
            return mysu_ioctl_handlers[i].handler(argp);
        }
    }

    pr_warn("mysu ioctl: unsupported command 0x%x\n", cmd);
    return -ENOTTY;
}

void __init mysu_supercall_dump_commands(void)
{
    int i;

    pr_info("MySU IOCTL Commands:\n");
    for (i = 0; mysu_ioctl_handlers[i].handler; i++) {
        pr_info("  %-18s = 0x%08x\n", mysu_ioctl_handlers[i].name, mysu_ioctl_handlers[i].cmd);
    }
}
