...
#define MODULE_RC_PATH_DEFAULT "/metadata/mysu/modules.rc"
...
// so we begin append mysu rc when we meet EOF.

static ssize_t read_proxy(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    ssize_t ret = 0;
    size_t append_count;
    if (mysu_rc_pos && mysu_rc_pos < mysu_rc_len)
        goto append_mysu_rc;
    if (mysu_rc_pos >= mysu_rc_len && module_rc_pos < module_rc_len)
...
append_mysu_rc:
    if (mysu_rc_pos < mysu_rc_len) {
        append_count = mysu_rc_len - mysu_rc_pos;
...
static void mysu_install_rc_hook(struct file *file)
{
    if (!is_init_rc(file)) {
        return;
    }
...
static void mysu_handle_sys_read(unsigned int fd, char __user **buf_ptr, size_t *count_ptr)
{
    struct file *file = fget(fd);
    if (!file) {
        return;
    }
    mysu_install_rc_hook(file);
    fput(file);
}
...
static void mysu_execve_hook_mysud_common(const char __user *filename_user, const char __user *const __user *argv_user)
{
    ...
    mysu_handle_execveat_mysud(path, &argv);
}
...
void mysu_execve_hook_mysud(const struct pt_regs *regs)
{
    ...
    mysu_execve_hook_mysud_common(filename_user, argv_user);
}
...
static long mysu_sys_fstat(const struct pt_regs *regs)
{
    ...
    if (is_rc) {
        ...
        long size, new_size;
        size_t extra = mysu_rc_len + module_rc_len;
        if (!copy_from_user_nofault(&size, st_size_ptr, sizeof(long))) {
            new_size = size + extra;
            pr_info("adding rc len: %ld -> %ld (static=%zu module=%zu)", size, new_size, mysu_rc_len, module_rc_len);
...
static int input_handle_event_handler_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    return mysu_handle_input_handle_event(type, code, value);
}
...
static void stop_init_rc_hook()
{
    mysu_syscall_table_unhook(__NR_read);
    mysu_syscall_table_unhook(__NR_fstat);
...
// mysud: module support
void __init mysu_mysud_init()
{
    int ret;

    mysu_syscall_table_hook(__NR_read, mysu_sys_read, &orig_sys_read);
    mysu_syscall_table_hook(__NR_fstat, mysu_sys_fstat, &orig_sys_fstat);

    ret = register_kprobe(&input_event_kp);
    pr_info("mysud: input_event_kp: %d\n", ret);

    INIT_WORK(&stop_input_hook_work, do_stop_input_hook);
}

void __exit mysu_mysud_exit()
{
    // TODO:
