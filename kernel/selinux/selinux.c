...
u32 mysu_file_sid __read_mostly = 0;
...
void cache_sid(void)
{
    ...
    err = security_secctx_to_secid(MYSU_FILE_CONTEXT, strlen(MYSU_FILE_CONTEXT), &mysu_file_sid);
    if (err) {
        pr_warn("Failed to cache mysu_file SID: %d\n", err);
        mysu_file_sid = 0;
    } else {
        pr_info("Cached mysu_file SID: %u\n", mysu_file_sid);
    }
}
...
{
    return is_task_mysu_domain(current_cred());
}
