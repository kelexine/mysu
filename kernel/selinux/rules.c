...
void apply_mysu_rules()
{
    ...
    backup_sepolicy =
        mysu_dup_sepolicy(rcu_dereference_protected(old_pol, lockdep_is_held(&selinux_state.policy_mutex)));
    if (IS_ERR(backup_sepolicy)) {
        ...
        if (!backup_sepolicy->sidtab) {
            ...
            mysu_destroy_sepolicy(backup_sepolicy);
            backup_sepolicy = NULL;
        } else {
    ...
    pol = mysu_dup_sepolicy(rcu_dereference_protected(old_pol, lockdep_is_held(&selinux_state.policy_mutex)));
    if (IS_ERR(pol)) {
...
{
    switch (cmd) {
    case MYSU_SEPOLICY_CMD_NORMAL_PERM:
        return 4;
    case MYSU_SEPOLICY_CMD_XPERM:
        return 5;
    case MYSU_SEPOLICY_CMD_TYPE_STATE:
        return 1;
    case MYSU_SEPOLICY_CMD_TYPE:
    case MYSU_SEPOLICY_CMD_TYPE_ATTR:
        return 2;
    case MYSU_SEPOLICY_CMD_ATTR:
        return 1;
    case MYSU_SEPOLICY_CMD_TYPE_TRANSITION:
        return 5;
    case MYSU_SEPOLICY_CMD_TYPE_CHANGE:
        return 4;
    case MYSU_SEPOLICY_CMD_GENFSCON:
        return 3;
    default:
        return -EINVAL;
    }
...
static int apply_one_sepolicy_cmd(struct policydb *db, const struct sepol_data *header, const char **args)
{
    ...
    switch (header->cmd) {
    case MYSU_SEPOLICY_CMD_NORMAL_PERM:
        if (header->subcmd == MYSU_SEPOLICY_SUBCMD_NORMAL_PERM_ALLOW) {
            success = mysu_allow(db, args[0], args[1], args[2], args[3]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_NORMAL_PERM_DENY) {
            success = mysu_deny(db, args[0], args[1], args[2], args[3]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_NORMAL_PERM_AUDITALLOW) {
            success = mysu_auditallow(db, args[0], args[1], args[2], args[3]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_NORMAL_PERM_DONTAUDIT) {
            success = mysu_dontaudit(db, args[0], args[1], args[2], args[3]);
        } else {
    ...
    case MYSU_SEPOLICY_CMD_XPERM:
        ...
        if (header->subcmd == MYSU_SEPOLICY_SUBCMD_XPERM_ALLOW) {
            success = mysu_allowxperm(db, args[0], args[1], args[2], args[4]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_XPERM_AUDITALLOW) {
            success = mysu_auditallowxperm(db, args[0], args[1], args[2], args[4]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_XPERM_DONTAUDIT) {
            success = mysu_dontauditxperm(db, args[0], args[1], args[2], args[4]);
        } else {
            pr_err("sepol: unknown subcmd: %d\n", header->subcmd);
        }
        return success ? 0 : -EINVAL;
    ...
    case MYSU_SEPOLICY_CMD_TYPE_STATE:
        ...
        if (header->subcmd == MYSU_SEPOLICY_SUBCMD_TYPE_STATE_PERMISSIVE) {
            success = mysu_permissive(db, args[0]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_TYPE_STATE_ENFORCE) {
            success = mysu_enforce(db, args[0]);
        } else {
            pr_err("sepol: unknown subcmd: %d\n", header->subcmd);
        }
        return success ? 0 : -EINVAL;
    ...
    case MYSU_SEPOLICY_CMD_TYPE_ATTR:
        ...
        if (header->cmd == MYSU_SEPOLICY_CMD_TYPE) {
            success = mysu_type(db, args[0], args[1]);
        } else {
            success = mysu_typeattribute(db, args[0], args[1]);
        }
        if (!success) {
    ...
    case MYSU_SEPOLICY_CMD_TYPE_TRANSITION: {
        ...
        success = mysu_type_transition(db, args[0], args[1], args[2], args[3], object);
        return success ? 0 : -EINVAL;
    }
    ...
    case MYSU_SEPOLICY_CMD_TYPE_CHANGE:
        ...
        if (header->subcmd == MYSU_SEPOLICY_SUBCMD_TYPE_CHANGE_CHANGE) {
            success = mysu_type_change(db, args[0], args[1], args[2], args[3]);
        } else if (header->subcmd == MYSU_SEPOLICY_SUBCMD_TYPE_CHANGE_MEMBER) {
            success = mysu_type_member(db, args[0], args[1], args[2], args[3]);
        } else {
            pr_err("sepol: unknown subcmd: %d\n", header->subcmd);
        }
        return success ? 0 : -EINVAL;
    ...
    case MYSU_SEPOLICY_CMD_GENFSCON:
        ...
        if (!mysu_genfscon(db, args[0], args[1], args[2])) {
            pr_err("sepol: %d failed.\n", header->cmd);
            return -EINVAL;
        }
        return 0;
...
int handle_sepolicy(void __user *user_data, u64 data_len)
{
    ...
    if (IS_ERR(pol)) {
        ret = PTR_ERR(pol);
        pr_err("mysu_dup_sepolicy err: %d\n", ret);
        goto out_unlock;
    }
    db = &pol->policydb;
