...
#define mysu_hash_for_each(node_ptr, n_slot, cur)                                                                       \
    int i;                                                                                                             \
    for (i = 0; i < n_slot; ++i)                                                                                       \
...
#define mysu_hashtab_for_each(htab, cur) mysu_hash_for_each(htab.htable, htab.size, cur)
#else
#define mysu_hashtab_for_each(htab, cur) mysu_hash_for_each(htab->htable, htab->size, cur)
#endif
...
#define avtab_for_each(avtab, cur) mysu_hash_for_each(avtab.htable, avtab.nslot, cur);
                         ...
                         struct perm_datum *perm, int effect, bool invert)
{
    ...
    if (src == NULL) {
        ...
        if (strip_av(effect, invert)) {
            mysu_hashtab_for_each(db->p_types.table, node)
            {
        ...
        struct hashtab_node *node;
        mysu_hashtab_for_each(db->p_classes.table, node)
        {
...
#define mysu_kvrealloc(p, new_size, _old_size) kvrealloc(p, new_size, GFP_KERNEL)
...
#define mysu_kvrealloc(p, new_size, old_size) kvrealloc(p, old_size, new_size, GFP_KERNEL)
#else
...
static bool add_type(struct policydb *db, const char *type_name, bool attr)
{
    ...
    struct ebitmap *new_type_attr_map_array =
        mysu_kvrealloc(db->type_attr_map_array, value * sizeof(struct ebitmap), (value - 1) * sizeof(struct ebitmap));
    ...
    struct type_datum **new_type_val_to_struct = mysu_kvrealloc(
    ...
    char **new_val_to_name_types =
        mysu_kvrealloc(db->sym_val_to_name[SYM_TYPES], sizeof(char *) * value, sizeof(char *) * (value - 1));
